...
## 2026-09-15 — Team slots: 7-day trend, and the first EV history we've ever kept

Brody asked for "trending up or down (sales of the cards within the team slot)" on the team slots table. **It could not be answered from the schema.** `pricing_cache` is `UNIQUE` on `player_product_id` and upserted on every refresh; `ch_price_cache` overwrites the same way. Every EV in the system is a single current value that gets overwritten nightly, so there was no past to compare against — and calling CardHedger at render time is exactly the pattern the pure-cache-read pricing architecture exists to prevent (504s). **So this adds the smallest possible history:** new `player_product_ev_snapshots` (`player_product_id`, `captured_on` date, `ev_mid`; PK on the pair), written by [pricing-refresh](lib/pricing-refresh.ts) in the same flush that upserts `pricing_cache` — **no extra CardHedger calls**, and repeat cron firings on one day overwrite rather than duplicate. The write is best-effort: a snapshot failure logs and never fails a pricing refresh. **Reads go through a `team_ev_trend(product_id, days)` RPC**, not a table scan — an active product can carry thousands of `player_products`, which would hit PostgREST's 1000-row cap and need `.in()` chunking under Kong's 200-UUID limit (gotcha #11); the function aggregates server-side and returns one row per team. **The correctness detail that matters:** `ev_now` is summed over exactly the player_products that have a historical row, because summing today's full roster against a smaller past set would report roster growth as price movement. UI is a new `7d` column — green up arrow, red down arrow, flat dash under ±1%. **Teams render `—` until history exists**, which means the column is empty on deploy and starts filling after the first nightly refresh; a real 7-day number takes a week. Migration applied to prod 2026-09-15 (`REVOKE` on the table, `REVOKE EXECUTE` + pinned `search_path` on the SECURITY DEFINER function, `NOTIFY pgrst` per gotchas #10/#12). **Caveat on the ask:** this trends our *modeled card value*, not sales volume — we don't store a sales count per card, so "are these selling more" is a different (unbuilt) metric.

## 2026-09-15 — Team slots: model value vs market price, deal check removed

Brody on the product page's team slots: *"the break price / signal … doesn't make any sense here. What we really need is what the cost of that team should be based on the configuration … what I'd actually rather have is fair market value."* **Removed** the `Break Price / Signal` column — the per-team ask input and its BUY/HOLD/PASS badge. That column asked the user to type a breaker's ask on a page whose job is stating what the spot is worth; the deal check still lives on Research and in the inline analysis block further down the same page. **Replaced** the single `Slot Cost` column with the two numbers that were always behind it: **Model Value** (the pure engine EV for that team at the current case configuration) and **Market Price** (that number × the lifecycle markup — 1.40 pre-release / 1.20 live / 1.05 dormant — i.e. what a breaker should be charging). Previously the market number was the headline and the model number was a 10px grey sub-line, which meant the more defensible of the two was the one nobody could read. Expanded player rows show the same split. **Also removed with the ask input:** the `Use $X` prefill pill and the `vs herd` chip, both of which wrote into or read from it. The read-only `Breakers asked $X–$Y · N obs · Nd ago` sub-row **stays** — it's market context that needs no input. Display-layer only: no engine, `pricing_cache` or API change, and `computeSignal` is untouched for its other callers.

## 2026-09-15 — Home is two doors: Value a spot, Log a break

Brody on #248: *"the value a spot inevitably takes you to [Research] where it's configuring something … so why not just have a CTA that takes you there anyway? If the line is 'BreakIQ does two jobs' … then we should just be directing you to those pages."* He was right. The quick-value form (#247) collected a product and an ask, then dropped you into Research's configurator to pick teams and cases anyway, so it was a slower way to get to the same page. **Home is now the headline made literal:** two equal job cards, **01 · Before you buy → Value a spot** (`/analysis`) and **02 · After you buy → Log a break** (`/my-breaks?view=new`). Each card has an icon, a title, one line on what the job does, and a button. Log keeps the page's only filled button, since logging is what feeds the comp set. The separate record panel is gone; once something is logged, a compact `N logged · N awaiting results → Your breaks` row sits inside the Log card, the job it belongs to. Zero-log accounts see no counts. Deleted `QuickValue.tsx` and removed its `?productId=&ask=` prefill from [AnalysisClient](app/(consumer)/analysis/AnalysisClient.tsx), which nothing else used. Home also drops its active-products fetch, so it's one less cache read per load.

## 2026-09-15 — Home: visual hierarchy so sections stop blending

Brody: *"the Home is still lacking some visual aspects that make everything blend together."* This time it was rendered and looked at first (local dev server, desktop 1440, mobile 390, dark and light), not reasoned from code. **What was wrong:** every section sat directly on the page background, split by the same 1px hairline; type sizes all fell in 13–16px; **two filled buttons competed** ("Log now" and "Value it"); desktop showed *Value a spot* twice (the quick-value form, then a route row under "Or"); and the page ended about 40% of the way down, with the gambling banner floating mid-screen. **Fix (UI only, no data or behavior changes):** the header stays on the background and everything you can act on moves onto raised **panels** (`--panel` + `--rule` border). **Log** is the main panel: a left key-line, an icon tile, a 19px title, and the page's **only filled button**. **Value a spot** becomes its own titled panel with an outlined button. **Your record** moves to a right column that matches the left stack's height. On desktop the "Or" route rows are gone (the rail and the quick-value panel already cover them); mobile keeps them, grouped in one panel. **Figures only show once something is logged** — #247 found every beta user except Brody has zero breaks, so a zero-log account sees "Nothing logged yet. Your first break starts the record." instead of a big `0 / 0`. **Layout:** the consumer shell is now a min-viewport-height flex column, so the responsible-gambling banner stays at the bottom of short pages on every consumer route.

## 2026-09-15 — Desktop Home gets a job: quick-value entry (mobile unchanged)

Brody on the new Home: *"I do like the simplicity … but nothing is grabbing my attention … it makes sense on mobile, from a web perspective it doesn't."* **Diagnosis: on desktop, Home duplicates the rail.** Its only job was routing to Research and Breaks, and the 196px rail does exactly that on every page — so there was nothing on desktop Home to grab attention *with*. Mobile works because the tab bar is four terse icons, so a stacked launcher with real labels earns its place. **Fix:** give Home a job the rail can't do — start a valuation. New [QuickValue](app/(consumer)/QuickValue.tsx) (desktop-only, `hidden lg:block`) takes a product + optional asking price and hands off to Research prefilled via `?productId=&ask=`; [AnalysisClient](app/(consumer)/analysis/AnalysisClient.tsx) reads those params, the same URL-prefill pattern `my-breaks` already uses for `?view=`/`?productId=`. **It deliberately does not run the analysis** — a verdict needs product + team/player selection + ask + ≥1 case, and building all of that on Home would rebuild Research there and recreate the duplication being fixed. **Rejected: a figure-led stat band.** Checked the data first — every beta user except Brody has **zero** logged breaks (Ted 0, Crease St Garden 0, Brody 4), so big dashboard numbers would render `0 / 0 / —` for every real user. Same trap as Learn. The quick-value entry is useful on day one with nothing logged, which is the state every new user is in. Also extracted the cached active-products fetch to [lib/active-products.ts](lib/active-products.ts) so Home's picker and Research's grid share **one** cache entry rather than each running the queries — adding the picker costs zero extra DB reads, which matters given prod is IO-constrained.

## 2026-09-15 — Research: Active Products was invisible (three causes, one symptom)

Brody reported "a really large gap in the research page" and "I didn't even know Active Products was there." Both were the same bug, plus a second one neither of us had spotted. **(1) `min-h-screen` on the wrong element.** [AnalysisClient](app/(consumer)/analysis/AnalysisClient.tsx) kept the `min-h-screen` it had when it was the entire page — but after the IA merge it's the top *section* of Research, with Active Products below. It forced a full viewport of height regardless of content and pushed the grid completely off-screen. That's the gap, and it's why the section was undiscoverable. **(2) Two headers.** [ActiveProductsBrowser](app/(consumer)/ActiveProductsBrowser.tsx) already renders its own "Active Products" heading; the wrapper added in #243 rendered a second one above it, so the page showed the title twice (visible in Brody's screenshot). Removed the wrapper's, kept the component's. **(3) The search already existed.** The browser has had search plus sport/year/lifecycle filters all along — Brody's "maybe we add search all products" was already built, just buried under a viewport of dead space. Nothing to add. The section header is now deliberately heavier than a plain `h2` — mono eyebrow, real title, and a live count (`11 · 10 live · 1 pre-release`) that both states the scale and implies the filters are worth using. **Rejected: moving Active Products to its own page.** With 11 active products it's a list, not a catalog; a dedicated page would spend a fourth destination immediately after #243 consolidated to three, and would put the product list a navigation away from the tool that consumes it. Worth revisiting past ~50 products — the browser component is self-contained, so extraction stays cheap.

## 2026-09-15 — Research product grid: cache the data, not the render (first tag-based invalidation)

Chasing a reported staleness bug found the opposite problem. **The claim that admin product edits took up to 60s to appear on Research was wrong** — verified against the build's `prerender-manifest.json`, where `/analysis` is absent, i.e. **dynamic**. The consumer layout calls `getCurrentUserFromSession()` → `cookies()`, which opts the whole segment into dynamic rendering and makes any page-level `revalidate` **inert**. Edits already appeared immediately. The old `/breaks` page carried `revalidate = 60` plus a comment claiming "one set of queries per minute, cached HTML served from edge for the next ~60 navigations" — **that was never true** once the layout started reading cookies, and no `revalidatePath('/')` ever existed in `app/admin` either. **The real cost:** because the render was never cached, *every navigation* re-ran three Supabase queries (products, 7-day break counts, active hype tags) — meaningful given prod is IO-constrained ([Supabase Disk IO](docs/BACKLOG.md)). **Fix:** cache the *data* rather than the render — `getProducts` now sits behind `unstable_cache` (60s, tagged `ACTIVE_PRODUCTS_TAG` from new [lib/cache-tags.ts](lib/cache-tags.ts)), and the four product-shape mutations (`createProduct`, `updateProduct`, `setProductLifecycle`, `deleteProduct`) call **`updateTag`** to drop it instantly. Safe to cache: the fetch touches only `supabaseAdmin`, no cookies/headers, and the grid is identical for every logged-in user. Used `updateTag` rather than `revalidateTag` because Next 16 changed the latter to require a cache-life profile, while `updateTag` is single-argument and documented for **server actions with read-your-own-writes** — exactly this case. This is the app's **first tag-based invalidation**; three existing comments in `pricing-read.ts`, `break-page-data.ts` and `player-comps` say "we could bust via `revalidateTag` if we wire it up" and none had. The misleading `revalidate` directive is gone, replaced by a comment recording why the route is dynamic so the next person doesn't re-derive it.

## 2026-09-15 — IA merge: Log + Breaks become one destination; Active Products moves to Research

Brody's review of the new nav produced three IA changes (PR #243). **(1) Log and Breaks are the same idea** — the historical record of every break you analyzed and bought — so they merged into ONE destination labelled **Breaks** at `/my-breaks`, keeping today's log functionality as the primary page. Primary nav drops from four items to **three: Home / Research / Breaks**; the pending-count badge moved with it. This supersedes the P2 resolution that filled Learn's dead slot with a Breaks product grid. **(2) The separate `/breaks` grid is deleted** and its **Active Products** section moved to **Research, below the two boxes** — the grid belongs next to the tool that values a spot, not on its own destination. That required `/analysis` to become a server wrapper ([page.tsx](app/(consumer)/analysis/page.tsx)) around the unchanged client tool ([AnalysisClient.tsx](app/(consumer)/analysis/AnalysisClient.tsx)), because the grid needs a richer query than the client's `/api/analysis` products list (7-day break counts + active hype tags). **(3) "Configure Your Bundle" and "AI Analysis" are now equal height** — they were independent grid columns so the shorter floated; both are flex columns whose `ElevatedCard` grows, and the empty state fills rather than sitting at a fixed `p-12`. **Preserved, and worth calling out: the `1-800-GAMBLER` responsible-gambling notice existed ONLY on `/breaks`.** Deleting that page would have removed the app's only compliance notice, so it became [ResponsibleGamblingBanner](components/breakiq/ResponsibleGamblingBanner.tsx) in the consumer layout and now renders on every consumer surface instead of one. Also corrected a stale comment inherited from `/breaks` claiming admin mutations invalidate the grid via `revalidatePath('/')` — no such call exists anywhere in `app/admin`, so the 60s ISR window is a real ceiling on how long a product edit takes to appear, not a backstop. Home's second route row changed from "Browse breaks" (the deleted grid) to "Your break record" → `/my-breaks`, so Home routes to the other two destinations.

## 2026-09-15 — UX rethink phases 1–5 shipped (UI-only)

Built the UI-only rethink across six PRs (#237–#242). **P1 — design foundation** (#237): Inter/JetBrains → **Public Sans + Roboto Mono**; the handoff's palette added as the source with the existing `--terminal-*`/`--text-*`/`--signal-*` names **re-pointed** at it, so ~93 files inherited the new look untouched (chosen over a mass find-and-replace); gradients flattened, glows → `none`, signal colors desaturated; light palette defined via `[data-theme="light"]`. Two deliberate restraints: **dark stays the default** (wiring `prefers-color-scheme` would hand every light-mode-OS user a different app — a behavior change, not a UI one), and `--btn-legacy-bg` keeps existing primary buttons legible since their labels hard-code white text. **P2 — nav + IA + Home** (#238): desktop 196px rail + mobile four-item tab bar, Slabs/Chase demoted to `TOOLS`; `/` becomes Home; the product grid moves to `/breaks` (`git mv`) and is middleware-gated. **The handoff's fourth destination is Learn, which isn't built — that slot is Breaks instead**, so there are four real destinations and nothing orphaned. Home ships **reduced on purpose**: the pickup block, stale markers and "valuations this month" all need a valuation history that `/api/analysis` never persists, and substituting recent *breaks* for recent *valuations* would misrepresent them. **P3 — Research** (#239): the filled, `border-2`, color-washed verdict card became the handoff's ruled three-field band; new shared `signalLabel()` maps `WATCH` → "HOLD" at the **display layer only** (persisted `snapshot_signal` untouched); and **262 hardcoded saturated `rgba()` literals were swept across 52 files** — call sites CSS variables could never reach, which P1's re-point had left saturated. Hex literals were deliberately *not* swept: the sport keys share values with the old signal colors and are intentionally saturated identity colors. **P4** (#240): finished the HOLD mapping on three surfaces P3 missed, including a *local* `SignalBadge` in `BreakerComparison` shadowing the shared one — P3 had made the same verdict read "HOLD" in one place and "WATCH" in another. **P5** (#241): fixed a regression P2 introduced — the four-item tab bar orphaned Slabs, Chase, Profile, Admin and **sign-out** on mobile (they lived in the removed slide-out sheet); the mobile header gains a compact overflow sheet. **P6** (#242): one scoped CSS rule corrects `min-h-screen` inside the padded shell so mobile pages don't carry ~72px of dead scroll, plus the plan's shipped-scope status. **Not built:** Log's table restyle (its key column derives from data that doesn't exist, and the 1,756-line file can't be visually verified from here), onboarding as designed (a live valuation before the paywall is new behavior, not a re-skin), and Learn (unscheduled, out of the nav). **Nothing auth-gated was visually verified** — preview deployments sit behind Vercel SSO and consumer routes need a session, so Home, the rail, Research and Log were verified by build + typecheck + routing only; worth a human pass.

## 2026-09-15 — UX rethink plan re-scoped to UI/UX-only

Brody scoped the rethink to **UI/UX only — where the design conflicts with current code, current code wins**. Revised [the plan](docs/plans/2026-09-15-ux-rethink-build-plan.md) accordingly; the constraint removes roughly half the designed surfaces, so the revision states exactly which and why. The previous draft's Phase 1 (capture returned value) is **cut** — it was a schema change. Three verified data dependencies gate the blocked half: (1) no returned/pull-value column on `user_breaks` (`outcome` is categorical `win|mediocre|bust`), blocking Log's Returned/Net columns and every Learn dollar figure; (2) **`/api/analysis` never persists a run — it only reads `products`, and no valuation/analysis table exists in `information_schema` or any migration** — which blocks Research's entire *Recent valuations* tab, Home's pick-up-where-you-left-off block, *"valuations this month"*, and every stale marker; (3) the engine emits a pipeline decomposition rather than per-input weights, so Research's weight bars can't be sourced. A fourth soft gap: *"saved by passing on spots above value"* counts spots the user walked away from, and we only ever record purchases — blocked on an uncaptured behaviour, not a column. **Learn is not scheduled** and recommended out of the nav (three destinations beats a dead fourth). What survives in full: design foundation (Public Sans/Roboto Mono, token re-point over a 93-file find-and-replace, light theme, no glows), nav/IA restructure, and Research's *Value a spot* — which re-skins the existing `/analysis`. Adapted rather than dropped: the method table re-skins [WhyThisPriceCard](components/breakiq/WhyThisPriceCard.tsx)'s existing five-layer decomposition (real numbers, no engine change), and Log swaps the Returned column for `snapshot_fair_value` ("what it was worth when you bought it"), keeping the row shape. `WATCH`→"hold" stays a display-layer mapping so persisted `snapshot_signal` values are untouched. The smallest unlock for the blocked half is recorded as two additive schema changes, explicitly not scheduled.

## 2026-09-15 — Phased build plan for the UX rethink (plan only)

Turned the design handoff into a sequenced build plan at [docs/plans/2026-09-15-ux-rethink-build-plan.md](docs/plans/2026-09-15-ux-rethink-build-plan.md), grounded in what the repo actually has rather than what the handoff assumes. **Nothing built.** Two blockers surfaced that the handoff assumes away: (1) **the app has never captured what a break returned** — `user_breaks` has `ask_price` but no returned/pull-value column (grep across `app/`/`lib/`/`migrations` finds nothing), and `outcome` is categorical `win|mediocre|bust`, so the Log table's Returned column and *every* Learn metric are underivable; this is the same gap blocking the north-star `pull_value / ask_price`, which makes capturing it Phase 1 and the highest-value phase in the plan despite not being design work. (2) **Prod has 4 breaks, 3 completed** — Learn's "11 of 14 calls held" / "$3,400 saved" would be noise, so Learn is gated on ~25 completed-with-returned-value across 2+ formats rather than a date. Also caught a naming collision the handoff didn't: `Signal` is `'BUY'|'WATCH'|'PASS'` in code and `WATCH` is *persisted* in `user_breaks.snapshot_signal`, while the design says HOLD throughout — recommendation is a display-layer mapping, not a data migration. Phases: capture returned value → design foundation (fonts/tokens/light theme; 93 files touch `var(--…)`, so re-point existing token names rather than find-and-replace) → IA + Home → Research → Log → Learn (gated) → onboarding (carrying forward the #228/#232 enforcement). Seven decisions are listed as blocking, including what happens to `/break/[slug]` and `/player/[id]` — neither appears in the new four-item IA.

## 2026-09-15 — Added the UX rethink design handoff to the repo

Checked in the designer's handoff bundle at [docs/design/2026-09-15-ux-rethink-handoff/](docs/design/2026-09-15-ux-rethink-handoff/README.md) so it lives with the code it describes and future sessions can find it via the CLAUDE.md docs index. It's a full consumer-app restructure: a four-item IA (Home / Research / Log / Learn, with Slabs + Chase demoted to a "Tools" group), a new Home surface whose primary ask is logging (because logged breaks feed the comp set), a formalized quieter visual system with complete dark/light token tables and typography specs, and a 5-step mobile onboarding that runs one real valuation before any profile question or the paywall. Three conceptual changes matter more than the pixels: no live-break framing anywhere (breaks have no timing data — rows show analysis facts, not schedules), "Analyses" and "Research" merged into one destination with two tabs, and logging promoted to the app's primary ask. **Nothing built yet** — this is reference material. The `.dc.html` files are HTML prototypes to recreate with the app's existing patterns, not code to port; `BreakIQ Hi-Fi v2.dc.html` is the one to build from (the others are superseded passes and tone studies kept for context). Five open product questions are listed in the README (Home empty state, stale threshold, Learn metrics, theme default, chart implementation).

## 2026-08-31 — Consumer P2 polish: dead ends, stuck spinners, missing paths

Six P2s from the funnel audit, one PR. **Sign-in had no password path** — [SignupForm](app/auth/signup/SignupForm.tsx) lets people create an email+password account, but [SigninForm](app/auth/signin/SigninForm.tsx) only offered a magic link, unexplained; new "Signed up with a password? Use it instead" toggle → `signInWithPassword` → full navigation to `/` so middleware reads the fresh cookies. **Signup "Check your email" was a dead end** — no resend, no way back; added *Didn't get it? Resend* (`auth.resend({ type: 'signup' })`) and *Use a different method*. **Subscribe checkout could stick on "Redirecting…"** — a response with neither `error` nor `url` fell through without clearing `loading`; it now throws into the existing catch ([subscribe/page.tsx](app/(consumer)/subscribe/page.tsx)). **Profile fetch had no error branch** — a 401/500 rendered an empty form as if the user had no data; now checks `res.ok` and shows an error ([profile/page.tsx](app/(consumer)/profile/page.tsx)). **`/profile` was missing from the middleware matcher** — the only consumer route relying solely on the layout guard; added to the matcher + `isConsumerRoute` ([middleware.ts](middleware.ts)). **Consumer empty state showed internal instructions** — "Add sports and products via the Supabase dashboard" → "No breaks are live right now — check back soon" ([page.tsx](app/(consumer)/page.tsx)). Not done here: the InstallPrompt/onboarding overlap was already fixed by the #228 route-group move. No schema, no migration.

## 2026-08-31 — Onboarding: age gate is final, CTA says where it goes, no step-1 flash

Three items from the funnel audit, all in [app/(onboarding)/onboarding/page.tsx](app/(onboarding)/onboarding/page.tsx). **(#11) The under-18 answer is final.** Clicking "No" showed the rejection banner but left "Yes, I'm 18+" fully clickable with no exit — the gate was trivially reversed and an honest under-18 user's only live control was the one that lies. Now "No" hides the Yes/No pair and offers **Sign out** (reuses the existing `logout` server action + `posthog.reset()`). The server already 403s on `is_over_18: false` (#228); this closes the client half. **(#12) The final button says where it goes.** It read "Show me my dashboard" but routed to `/subscribe` (the plan picker) — a promise/destination mismatch. Relabeled **"Pick your plan"**; the route is unchanged (the plan-picker-after-onboarding funnel is a deliberate product choice, so this is a copy fix, not a funnel change). **(P2) No step-1 flash.** The "already completed?" probe had no loading state, so a returning user saw step 1 for a beat before the redirect, and an incomplete user saw the legal checkbox pop in after first paint. New `probing` state holds a minimal "Loading…" until `/api/profile` answers; a failed probe releases it (never traps the user). No schema, no migration.

## 2026-08-31 — Auth UX: no more silent failures on the sign-in surface

Closes out the sign-in surface from the funnel audit (#6, #7, #5) — the same **silent-failure** family that let the months-old OAuth bug hide. **(#6) OAuth errors are surfaced.** [components/auth/OAuthButtons.tsx](components/auth/OAuthButtons.tsx) discarded the `{ error }` from `signInWithOAuth`, so a blocked popup, misconfigured provider, or network failure meant "Continue with Google" did literally nothing — no spinner, no message. Now: per-button pending state ("Redirecting to Google…"), the error rendered inline, and a `try/catch` around the call. On success the browser navigates away, so pending intentionally stays set. **(#7) Returning users no longer dead-end.** [app/auth/callback/route.ts](app/auth/callback/route.ts) sent *every* failed sign-in (`session_failed` / `missing_code`) to `/auth/signup?error=…` — a returning user with an expired magic link was told to "try your invite link again" (they have none) with only "Back to waitlist" as an exit. Failures with no `invite_code` now route to `/auth/signin?error=…`; [app/auth/signin/page.tsx](app/auth/signin/page.tsx) reads `searchParams` (server-side, same pattern as signup) and [SigninForm](app/auth/signin/SigninForm.tsx) renders a reason + retry path — so the redirect target actually *shows* the error rather than becoming a new blank-form failure. The signup error state also gains a sign-in link. **(#5) Disabled buttons no longer hide the reason.** [SignupForm.tsx](app/auth/signup/SignupForm.tsx) had `disabled={!accepted}` on the OAuth + email buttons, so the click never fired, `guardAccepted()` never ran, and the "accept the Terms" message was unreachable dead code — four greyed-out buttons with no explanation. Buttons stay enabled; the guard surfaces the reason on click. No schema, no migration.

## 2026-08-31 — New-user funnel P1s: dead CTA, subscriber detection, submit hang, legal-acceptance gap

Four P1s from the same funnel audit, shipped as two PRs. **PR #229 (one-liners):** the welcome email's primary CTA linked `/break`, but only `/break/[slug]` exists — the first email every new user got had a dead "Browse upcoming breaks →" button; now links the home product grid ([lib/email.ts](lib/email.ts)). And `/subscribe` reads `profile.subscription_plan` to skip the plan picker for existing subscribers, but `/api/profile` never selected that column → always `undefined` → paying users got re-pitched Hobby/Pro; added it to the select ([app/api/profile/route.ts](app/api/profile/route.ts), column verified present). **This PR:** (1) the waitlist submit wasn't in a `try/catch`, so a dropped connection rejected the `fetch`, `setState('error')` was never reached, and the button sat on "Submitting…" until reload ([app/waitlist/page.tsx](app/waitlist/page.tsx)). (2) **Legal-acceptance gap opened by the 2026-08-27 email-fallback fix (#227):** users admitted via `/auth/signin` arrive with a bare `/auth/callback` carrying no `accept_*` params, so their profile got `terms_accepted_at = null` and they saw red "Not accepted" badges with no way to accept. Onboarding step 1 ([app/(onboarding)/onboarding/page.tsx](app/(onboarding)/onboarding/page.tsx)) now probes `/api/profile` and shows a Terms/Privacy checkbox **only when acceptance is missing** (defaults to over-ask until the probe returns — never under-capture); clicking "Yes, I'm 18+" without it surfaces the reason instead of silently disabling (the audit's #5 pattern). The PUT round-trips `accept_terms`/`accept_privacy` as **versions**, and [app/api/onboarding/route.ts](app/api/onboarding/route.ts) writes the four legal fields only when they match the current published versions and only if not already set — same rules as `/auth/callback`. No schema, no migration.

## 2026-08-31 — New-user funnel: stop silent bounces + make the onboarding gate real

Two P0s from a funnel audit (see the full list in that session's PR #228 description). Theme: **silent failures** — errors happened and the user was told nothing, which is exactly how the months-old OAuth bug stayed hidden. **(1) Waitlist explains the bounce.** [app/waitlist/page.tsx](app/waitlist/page.tsx) never read `?error=`, so a user the callback rejected (`missing_invite` / `invalid_invite`) landed on the bare "Request beta access" form with zero explanation, re-submitted, hit the 409 "already on the list," and looped (Ted Mann). New `<AuthBounceBanner>` (Suspense-wrapped `useSearchParams`, same pattern as `admin/login`) renders a clear reason + "Sign in with your approved email →". **(2) Onboarding — incl. the 18+ gate — is now enforced, not optional.** The callback's one-time redirect was the *only* enforcement; `ConsumerNav` rendered on top of the wizard, so a new user on step 1 could click any nav link to the dashboard and was never asked again. Fix: moved the wizard into its own route group **`app/(onboarding)/`** (was `app/(consumer)/onboarding/`, URL unchanged) with an auth-only layout — no nav, no `InstallPrompt` (also fixes the P2 mobile overlap) — and [app/(consumer)/layout.tsx](app/(consumer)/layout.tsx) now redirects any non-admin with null `onboarding_completed_at` to `/onboarding` (no loop, since it's outside the group). Server side, [app/api/onboarding/route.ts](app/api/onboarding/route.ts) now **403s on `is_over_18: false`** instead of accepting it and marking onboarding complete (the age gate was client-side theater), and uses `.select('id').maybeSingle()` so a zero-row update **404s** instead of returning `{ ok: true }` while writing nothing. No schema, no migration.

## 2026-08-27 — Auth: admit approved invitees by verified email (critical)

Fixed a beta-blocking auth bug: an **approved** invitee who authenticated from anything other than their exact invite link got bounced to the request-access screen. Root cause — the `/auth/callback` invite gate ([app/auth/callback/route.ts](app/auth/callback/route.ts)) keyed access **only** off the `invite_code` URL param, but the sign-in page ([SigninForm.tsx](app/auth/signin/SigninForm.tsx)) uses a bare `/auth/callback` with no code (by design, for returning users). So a first-time approved user who landed on `/auth/signin` (bookmark, "Sign in" link, or bouncing between signup/signin) authenticated successfully, hit `missing_invite`, and was signed back out — the "stuck on signin" report from beta user Ted Mann. Fix: the callback now grants access by **waitlist approval**, keyed off EITHER a valid `invite_code` OR the **OAuth-verified email** matching an approved waitlist row (`escapeLike` makes the `ilike` a true exact match so `_` in an address can't over-match). The email is provider-verified (Google/Discord), so an approved row for it is legitimate. The mark-converted step also resolves by email now, so email-path users still flip approved→converted (funnel stays accurate). No schema, no migration. Ted was hand-unblocked in prod (profile created) ahead of the fix. Comments on the sign-in surfaces updated to describe the new behavior.

## 2026-08-16 — Per-player-per-product EV override

A manual lever to correct a single player's base EV when the model gets it wrong — a prospect we expect to explode (thin comps, no market yet) or a base that doesn't match what Brody/Kyle know from the field. The override is the **base EV** (confirmed with Brody): it flows through pool weighting → display breaker markup → compression like any modeled EV, so the slot price ends up **higher** than the entered number — it does *not* pin the final slot price. New migration [20260816120000_player_products_ev_override.sql](supabase/migrations/20260816120000_player_products_ev_override.sql) adds nullable `ev_override` + `ev_override_note` / `ev_override_set_by` / `ev_override_set_at` to `player_products` (applied via MCP; additive nullable → no grants/RLS/NOTIFY). Applied at **read time** via one shared helper `evOverrideFor()` ([lib/ev-override.ts](lib/ev-override.ts)) in all four `PlayerWithPricing` builders — `loadCached` + `loadPreReleaseBaseline` ([lib/pricing-read.ts](lib/pricing-read.ts)), `runBreakAnalysis` ([lib/analysis.ts](lib/analysis.ts)), `getTeamFairValuesForProduct` ([lib/team-fair-value.ts](lib/team-fair-value.ts)) — which is the true single chokepoint since live reads `pricing_cache` and pre-release reads the column. Read-time (vs. a cache write) makes it authoritative regardless of refresh state (live the instant it's saved), durable across every refresh, and identical for live + pre-release; the engine / refresh pipeline / `pricing_cache` are untouched. `evMid` = override, `evLow/High` derived at 0.35 / 2.5. New `pricingSource: 'override'` on the type union. Admin UX: an **EV Override column** on the **Roster Sentiment** grid ([RosterSentimentEditor.tsx](app/admin/products/[id]/players/RosterSentimentEditor.tsx), `/admin/products/[id]/players`) — per-row `$` input next to the sentiment ± lever, modeled EV as the placeholder (`model 121`), blank clears. The grid's single **Save all** persists both levers in parallel: `saveBreakerzBets` (sentiment) + new `saveEvOverrides` ([actions.ts](app/admin/products/actions.ts)). Consumer [WhyThisPriceCard](components/breakiq/WhyThisPriceCard.tsx) shows "Manual override — base EV set by BreakIQ" and suppresses the lifecycle-math line. Plan: [docs/plans/2026-08-16-ev-override.md](docs/plans/2026-08-16-ev-override.md). _(Surface moved 2026-08-16: first shipped as a standalone dashboard section w/ its own search picker — `EvOverridesManager.tsx` + `/api/admin/ev-overrides`; Brody flagged the roster grid as the right home, so it moved onto the grid and the dashboard section + component + route were deleted.)_

## 2026-08-14 — Pre-release pricing Phase 2 (priced board)

The payoff — pre-release products now show a **priced slot board**, not just chase cards. New `loadPreReleaseBaseline` ([lib/pricing-read.ts](lib/pricing-read.ts)) builds the same `PlayerWithPricing` rows as `loadCached` but with EV from `pre_release_base_ev` (`pricingSource: 'pre_release_baseline'`, added to the type + PlayerTable's estimated styling); [break-page-data](lib/break-page-data.ts) picks that loader for pre-release products. [BreakPageClient](app/(consumer)/break/[slug]/BreakPageClient.tsx) renders a **"Projected Slot Pricing"** section (Team Slots / Player tables, teams↔players toggle) **below** the chase/hype layout — so the whole existing pipeline applies to the synthetic EV: sentiment (`breakerz_score` via `computeSlotPricing`), the lifecycle markup (1.40 pre-release), and compression + per-product γ. **Self-gating** on "any baseline present" (`p.pricingSource === 'pre_release_baseline'`), so a pre-release product with no baseline built renders byte-for-byte as before — the full roster loads via the same shape `loadCached` used (evMid=0), no regression. Validate by running the Phase 1 "Build pre-release baseline" on a product, then viewing its break page. Trend still v2 (icebox). PRD: [docs/plans/2026-08-14-pre-release-pricing.md](docs/plans/2026-08-14-pre-release-pricing.md).

## 2026-08-14 — Pre-release pricing Phase 1 (baseline synthesis)

The pre-release product finally gets a *price*, not just chase cards + comps. Pre-release has no EVs (unreleased → no CH sales/odds), so we synthesize a baseline per player from **last cycle + our read**: non-rookie → previous-cycle product's cached EV (new `products.previous_product_id` link) → else the player's 90-day raw comp (`pre_release_player_snapshots.raw_avg_90d`); rookie → rank-tiered floor (reuses `computeFallbackBaseEV`). Trend deferred to v2 ([icebox](docs/icebox.md)). New migration adds `products.previous_product_id` + `player_products.pre_release_base_ev`. Compute in [lib/pre-release-baseline.ts](lib/pre-release-baseline.ts); admin action `buildPreReleaseBaseline(productId, write)` ([actions](app/admin/products/actions.ts), preview→build); [BuildBaselineButton](app/admin/products/[id]/BuildBaselineButton.tsx) in a new **Pre-Release Baseline** section (pre-release products only) with a source-tallied preview table; **Prior-cycle product** picker on the product edit form. **Phase 1 is inputs only** — the baseline feeds nothing consumer-facing yet. **Phase 2** (render the priced slot board on the pre-release page from the synthetic EV, alongside the chase layout — so sentiment + compression apply) is next. No flag — the definitive pre-release path, self-gating on "baseline exists." Full plan: [docs/plans/2026-08-14-pre-release-pricing.md](docs/plans/2026-08-14-pre-release-pricing.md).

## 2026-08-14 — Per-product compression γ + flag flipped ON

Flipped `compression_markup_enabled` ON, and made the compression exponent **per-product** — because the market's slot-shape varies by product character (Brody: "certain products are worth much more just by the nature of their quality, desirability, and staying power"). Commodity products **compress** (γ<1); premium products where a singular marquee chase carries the break (Cosmic Chrome BB / Flagg, Cactus Jack) **amplify** the top (γ>1). Same reason sentiment is per-product. New `products.compression_gamma numeric` (nullable → global `COMPRESSION_GAMMA` 0.5); the compression fn already handles both directions from one number (γ<1 compress, 1 flat, >1 amplify). Read server-side on the break page (`product.compression_gamma ?? COMPRESSION_GAMMA`), set on the product edit form (**Pricing → Compression γ**), threaded through `createProduct`/`updateProduct`. **Interim:** the two known premium-hoops products set to **γ=1.0** (flat — safe, not mis-compressed against their nature) until their amplify value is dialed in. Migration `20260814…_products_compression_gamma.sql` (via MCP). Verified on live data: Cosmic Chrome Football big teams dampen 1.20→~0.85, small floor 1.20→~1.67, total conserved. PRD updated: [docs/plans/2026-08-14-market-compression-markup.md](docs/plans/2026-08-14-market-compression-markup.md).

## 2026-08-14 — Market compression markup (flag-gated)

Builds the [breaker-markup validation](docs/breaker-markup-validation.md) into the model. The market prices break slots **flatter than our EV** — it floors small spots (Kyle's Fanatics number: ≈ case-cost-share + ~10%) and dampens the top — but our display markup (`MARKET_MARKUP_BY_LIFECYCLE`) applies a **flat** multiplier to every slot. New `compressionMarkups()` in [lib/market-markup.ts](lib/market-markup.ts) reallocates a break's flat markup across its slots via power compression: `perSlotMarkup_i = M · s_i^(γ−1) / Σ s_j^γ` (s = EV share). **Conserves each break's total** — a redistribution, not an inflation — so the bundle verdict is unchanged; only per-slot prices move. `γ=1` is the exact identity; ships at `γ=0.5` (data implies ~0.35, tune via Market Delta). Verified: big slots dampened, small floored, total conserved.

**Scope:** display layer only — the `/break/[slug]` **Team Slots** + **Player** tables ([TeamSlotsTable](components/breakiq/TeamSlotsTable.tsx), [PlayerTable](components/breakiq/PlayerTable.tsx)). Engine, `pricing_cache`, math-layer multiplier, and the bundle verdict untouched. **Flag-gated:** `feature_flags.compression_markup_enabled` ([lib/feature-flags.ts](lib/feature-flags.ts), seeded OFF); read server-side on the break page, passed to the tables as `compressionGamma`. Off → byte-for-byte identical (γ=1). PlayerDetailDrawer keeps flat markup in v1 (single-slot). Rollout: deploy off → flip via SQL → validate → tune γ; per-product γ (amplify for premium hoops with a singular chase) is the follow-up. Full PRD: [docs/plans/2026-08-14-market-compression-markup.md](docs/plans/2026-08-14-market-compression-markup.md).

## 2026-08-14 — Test/real break flag (keep dry-run breaks out of analytics)

Most logged breaks so far are test entries with placeholder numbers, which pollute Market Delta (Section 1 reads `user_breaks` ask vs. our snapshot). New `user_breaks.is_test boolean not null default false` ([migration](supabase/migrations/20260814180000_user_breaks_is_test.sql)). My Breaks form gets a **Test break** toggle (sticky per device via `localStorage` — testers log many in a row); `POST /api/my-breaks` writes it; break cards show a small **TEST** badge; Market Delta Section 1 excludes `is_test = true`. Default false so existing + real breaks are unaffected. Note this only cleans the *logged-break* side — the `/break-price` competitor captures (Section 2, used in the markup validation) were already real. Companion to the breaker-markup work; also appended the Fanatics floor number (small-break floor ≈ case-cost-share + ~10%) to [docs/breaker-markup-validation.md](docs/breaker-markup-validation.md).
