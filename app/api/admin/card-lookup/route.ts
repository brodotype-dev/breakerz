...
Use empty strings for any field not visible or not applicable.`,
  ...
  if (body.action === 'cert') {
    ...
    // Compute summary stats from price history
  ...
  if (body.action === 'price') {
    ...
    const allPrices = pricesResult.status === 'fulfilled' ? pricesResult.value.prices : [];
    ...
    return NextResponse.json({
      source: 'search',
      card: {
        card_id: card.card_id, player_name: card.player_name, set_name: card.set_name,
        year: card.year, number: card.number, variant: card.variant, rookie: card.rookie,
      },
      allPrices,
      comps,
