...
**Next up:** Flip the slice 2b toggle on in prod and gut-check the enriched verdicts against real captures. If the narrative lands as helpful (not noise), leave on; if not, flip back off and we'll tune the prompt or the threshold. Then step #3 of the execution roadmap — side-by-side comparison UI on `/break/[slug]`. The per-team fair-value query that powers comparison also unblocks computing deltas on the captures panel inside Market Delta Watch. Then defensive layer (set-name validator widget + gated activation state machine per `~/.claude/plans/2026-05-11-data-accuracy-roadmap.md`), streaming pricing refresh before product count crosses 25, and Grade Ratio Value when Kyle confirms CH Q13. Phase 3c — variant-aware engine reads (resolve variant_name → variant_id, apply variant hype as EV multiplier, override `*_odds` from active odds_observations). Slice 2a (calibration aggregator that adjusts engine markup constants from observed asks) deferred until 2b validates data quality. Phase 5 C-score (blocked on Kyle), My Breaks Phase 2 (chase/hit card tracking), Sentry error tracking, rate limiting, 2025-26 Bowman Basketball re-match (CPA cards being added by CH this week)
